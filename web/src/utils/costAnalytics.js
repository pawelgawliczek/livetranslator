...
/**
 * Get multi-speaker overview statistics
 */
export async function getMultiSpeakerOverview(token, startDate, endDate) {
  const params = new URLSearchParams({
    start_date: startDate.toISOString(),
    end_date: endDate.toISOString(),
  });

  const response = await fetch(`${API_BASE}/api/admin/costs/multi-speaker/overview?${params}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch multi-speaker overview: ${response.statusText}`);
  }
...
/**
 * Get multi-speaker room details
 */
export async function getMultiSpeakerRoomDetails(token, roomCode) {
  const response = await fetch(`${API_BASE}/api/admin/costs/rooms/${roomCode}/multi-speaker-details`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch multi-speaker room details: ${response.statusText}`);
  }
...
/**
 * Get translation pairs for a multi-speaker room
 */
export async function getTranslationPairs(token, roomCode, limit = 20) {
  const params = new URLSearchParams({
    limit: limit.toString(),
  });

  const response = await fetch(`${API_BASE}/api/admin/costs/rooms/${roomCode}/translation-pairs?${params}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch translation pairs: ${response.statusText}`);
  }
...
/**
 * Get speaker activity for a multi-speaker room
 */
export async function getSpeakerActivity(token, roomCode) {
  const response = await fetch(`${API_BASE}/api/admin/costs/rooms/${roomCode}/speaker-activity`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch speaker activity: ${response.statusText}`);
  }
 ...
 */
export async function getCostTimeline(token, roomCode, granularity = 'hour') {
  const params = new URLSearchParams({
    granularity,
  });

  const response = await fetch(`${API_BASE}/api/admin/costs/rooms/${roomCode}/cost-timeline?${params}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
/**
 * Get optimization suggestions for a multi-speaker room
 */
export async function getOptimizationSuggestions(token, roomCode) {
  const response = await fetch(`${API_BASE}/api/admin/costs/rooms/${roomCode}/optimization-suggestions`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
