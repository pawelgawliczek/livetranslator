...
export default function WelcomeBanner({
  ...
  return (
    <div className="fixed top-[60px] left-1/2 -translate-x-1/2 bg-card border border-border rounded-xl px-6 py-4 shadow-2xl z-[998] max-w-[400px] w-[90%]">
      ...
      <div className="flex justify-between items-start mb-2">
        ...
        <button
          onClick={onClose}
          className="bg-transparent border-0 text-muted cursor-pointer text-xl p-0 leading-none hover:text-fg transition-colors"
            ...
            {otherParticipants.map(p => {
              ...
              return (
                <li key={p.user_id} className="mb-1">
                  ...
                  {p.is_guest && <span className="text-muted"> (guest)</span>}
                  <span className="text-muted"> ({lang?.name || p.language})</span>
                </li>
              );
            })}
          </ul>
