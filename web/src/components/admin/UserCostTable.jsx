  ...
  if (loading) {
    return (
      <div className="bg-card rounded-lg p-6 border border-border">
        <h4 className="text-lg font-semibold text-fg mb-4">👥 Top Users by Cost</h4>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left py-3 px-2 text-muted font-medium text-sm">User</th>
              <th
              ...
              <th className="text-right py-3 px-2 text-muted font-medium text-sm">Actions</th>
            </tr>
          </thead>
          <tbody>
            {users && users.length > 0 ? (
              users.map((user, index) => (
                <tr key={user.user_id} className="border-b border-border hover:bg-bg-secondary hover:bg-opacity-50">
                  <td className="py-3 px-2 text-muted text-sm">{page.offset + index + 1}</td>
                  <td className="py-3 px-2">
                    <div className="text-fg font-medium text-sm">{user.email}</div>
                    <div className="text-muted text-xs">{user.display_name}</div>
                  </td>
                  <td className="py-3 px-2 text-fg text-sm">{user.room_count}</td>
                  <td className="py-3 px-2">
                    <div className="text-fg text-sm font-medium">{formatCurrency(user.stt_cost_usd)}</div>
                    <div className="text-muted text-xs">{user.stt_minutes.toFixed(0)} min</div>
                  </td>
                  <td className="py-3 px-2">
                    <div className="text-fg text-sm font-medium">{formatCurrency(user.mt_cost_usd)}</div>
                    {user.mt_tokens && (
                      <div className="text-muted text-xs">{formatNumber(user.mt_tokens)} tok</div>
                    )}
                    {user.mt_characters && (
                      <div className="text-muted text-xs">{formatNumber(user.mt_characters)} char</div>
                    )}
                  </td>
                  ...
                  <td className="py-3 px-2 text-right">
                    <button
                      ...
                      className="px-3 py-1 bg-accent text-accent-fg rounded text-xs font-medium hover:bg-accent hover:bg-opacity-90 transition-colors"
                    >
                      View
                    </button>
                  </td>
