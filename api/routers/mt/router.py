                ...
                for tgt_lang in target_langs:
                    ...
                    try:
                        ...
                        if mt_mode == "openai":
                            ...
                            translated = await openai_translate(text, src_lang, tgt_normalized)
                            backend_name = "openai"
                        else:
                            # Local mt_worker only supports pl<->en
                            if tgt_normalized not in ["pl", "en"]:
                                print(f"[MT Router] ⚠ Local worker doesn't support {tgt_normalized}, using OpenAI")
                                translated = await openai_translate(text, src_lang, tgt_normalized)
                                backend_name = "openai"
                            else:
                                translated = await local_translate(client, text, src_lang, tgt_normalized, is_final)
                                backend_name = "local"
                        ...
                        print(f"[MT Router] ✓ {kind} ({backend_name}) {src_lang}→{tgt_normalized}: {translated[:60]}...")
